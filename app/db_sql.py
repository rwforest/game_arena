...
CATALOG = os.environ.get("GA_SQL_CATALOG", "unitygo")
SCHEMA = os.environ.get("GA_SQL_SCHEMA", "lichess")
TABLE = os.environ.get("GA_SQL_TABLE", "game_arena")
...
def init_table() -> None:
    ...
    if conn is None:
        return
    print("Initializing database table if not exists...")
    with conn:
...
def insert_game(*, experiment_id: str, run_id: str, white: str, black: str, model_white: str, model_black: str, result: str, pgn: str) -> None:
    ...
    if conn is None:
        return
    print(f"Inserting game result for run {run_id} into {FQN}...")
    with conn:
        with conn.cursor() as cur:
